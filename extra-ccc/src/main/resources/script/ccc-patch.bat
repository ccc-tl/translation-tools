...
CHOICE /C YN /M "Change Meltlilith to Meltryllis"
if %ERRORLEVEL% == 1 set "FLAGS=%FLAGS% --useMeltryllis"
...
fine.exe ccc.patchfs "%~1" ccc-patched.iso %FLAGS% > NUL 2>&1
if errorlevel 1 goto :FINEERR

echo.
echo Success!
echo Patched ISO was saved as ccc-patched.iso next to the bat file.
goto :FIN
